...
このように状態を持たない(attribute を持たない)クラスは、singleton にしてもテストのやりにくさと関係なく、かつ不要なオブジェクトを作成しないことでメモリを節約できるためメリットが勝つ。状態を持つクラスはメリットよりもデメリットの方が大きいため、singleton にしないパターンが多いのだと思う。
